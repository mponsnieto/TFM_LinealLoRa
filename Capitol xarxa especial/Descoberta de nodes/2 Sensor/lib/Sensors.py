...
class Comunication:
  ...
  def llegirSensors(self):
      ...
      #Sensor HS
      self.val,self.dry,self.humid,self.inWater=sensorHSol.CalcularHumitat()
      ...
      val=int(val)
      #Sensor QAire
      self.dustDensity=sensorQAire.CalculateDust()
      time.sleep(1)
      ...
      #Sensor HT
      self.T=SensorHT.readTemperature()
      time.sleep(0.3)
      packet_Tht = ustruct.pack('f',T)
      self.H=SensorHT.readHumidity()
      time.sleep(0.3)
     ...
     def check_alarms(self):
         ...
         if (int(self.T>50)+int(self.temp>50)+int(self.tempC>50))>=2:
             return True
         if (int(self.T>30)+int(self.temp>30)+int(self.tempC>30))>=2 and self.H<30 and self.dry:
             return True
         if cam.checkAlarm:
             return True
         if self.T>50 or self.temp>50 or self.tempC>50:
